# =============================================================================
#  xhttp-northflank :: VLESS + XHTTP origin
#
#  This image runs ONLY vless + xhttp over plain HTTP.
#  It deliberately does NOT do TLS: TLS is terminated by the Northflank edge
#  (in front of the container) and by Cloudflare (in front of Northflank).
#  Keeping the container free of TLS keeps its memory footprint small.
#
...
# Pinned Xray-core release. Bump via the XRAY_VERSION build arg / repo variable.
...
# No Docker HEALTHCHECK on purpose:
# Northflank probes the container itself. Use a *TCP* probe on PORT -
# an HTTP probe cannot work here (see README, "Health check").
