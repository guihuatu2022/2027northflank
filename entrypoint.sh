#!/bin/sh
# =============================================================================
#  xhttp-northflank :: entrypoint
#
#  Renders the Xray configuration from environment variables, validates it with
#  the real Xray binary, then execs Xray. Nothing about the config is hardcoded:
#  every tunable below can be set in the Northflank service environment.
# =============================================================================
set -eu
...
# Reject values that would break the JSON template we generate below.
safe_str() { # <name> <value>
  case "${2:-}" in
...
# Xray's JSON config accepts a range either as a plain integer (when from == to)
# or as a "from-to" string. It does NOT accept {"from": x, "to": y}.
...
# ---------------------------------------------------------------------------
TEST_LOG="$(mktemp)"
if ! xray run -test -config "$CONFIG_PATH" >"$TEST_LOG" 2>&1; then
  log "the generated config did not validate:"
  cat "$TEST_LOG" >&2
  rm -f "$TEST_LOG"
  fail "invalid Xray config - fix the environment variables listed above"
fi
rm -f "$TEST_LOG"

# ---------------------------------------------------------------------------
# keep the Go runtime inside a small container limit
# ---------------------------------------------------------------------------
...
[ "$XHTTP_HOST" = "" ] || log "warning: XHTTP_HOST='${XHTTP_HOST}' enables Host header validation - it must match exactly what Cloudflare sends"
